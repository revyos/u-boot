...
struct csi_sm3_t {
    ...
    void (*callback)(csi_sm3_t *sm3, csi_sm3_event_t event, void *arg); /*SM3 event callback for user*/
...
/**
  \brief       Initialize SM3 Interface. Initializes the resources needed for
  the SM3 interface \param[in]   sm3  operate handle. \param[in]   idx index of
  sm3 \return      error code \ref csi_error_t
*/
csi_error_t csi_sm3_init(csi_sm3_t *sm3, uint32_t idx);

/**
  \brief       De-initialize SM3 Interface. stops operation and releases the
  software resources used by the interface \param[in]   sm3  sm3 handle to
  operate. \return      none
*/
