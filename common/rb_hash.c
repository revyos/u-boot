...
#include <rambus/rambus_sha.h>

typedef struct rb_sha_context_t {
    csi_sha_t sha;
    csi_sha_context_t context;
} rb_sha_context;

static int __maybe_unused hash_init_sha256(struct hash_algo *algo, void **ctxp)
{
    rb_sha_context *ctx = malloc(sizeof(rb_sha_context));
    if (ctx == NULL)
        return -ENOMEM;
    csi_sha_init(&(ctx->sha), 0);

    csi_error_t ret = csi_sha_start(&(ctx->sha), &(ctx->context), SHA_MODE_256);
    if (ret != CSI_OK) {
        free(ctx);
        return -EFAULT;
    }
...
static int __maybe_unused hash_update_sha256(struct hash_algo *algo, void *ctx, const void *buf, uint size, int is_last)
{
    if (ctx == NULL)
        return -ENOMEM;
    csi_error_t ret = csi_sha_update((&((rb_sha_context *)ctx)->sha), &(((rb_sha_context *)ctx)->context), buf, size);

    if (ret != CSI_OK) {
        csi_sha_uninit(&((rb_sha_context *)ctx)->sha);
        free(ctx);
        return -EFAULT;
    }
    return 0;
...
static int __maybe_unused hash_finish_sha256(struct hash_algo *algo, void *ctx, void *dest_buf, int size)
{
    if (ctx == NULL)
        return -ENOMEM;
    ...
    csi_error_t ret =
            csi_sha_finish(&(((rb_sha_context *)ctx)->sha), &(((rb_sha_context *)ctx)->context), dest_buf, &size);
    csi_sha_uninit(&((rb_sha_context *)ctx)->sha);
    free(ctx);
...
static void sha256_csum(const unsigned char *input, unsigned int ilen, unsigned char *output, unsigned int chunk_sz)
{
    void *ctx;
    ...
    while (curr < end) {
        ...
        if (chunk > chunk_sz)
            chunk = chunk_sz;
        hash_update_sha256(algo, ctx, curr, chunk, 0);
        curr += chunk;
        schedule();
    }
#else
    hash_update_sha256(algo, ctx, input, ilen, 1);
#endif

    hash_finish_sha256(algo, ctx, output, SHA256_SUM_LEN);
}
...
static int __maybe_unused hash_update_crc32(struct hash_algo *algo, void *ctx, const void *buf, unsigned int size,
                                            int is_last)
{
...
static int __maybe_unused hash_finish_crc32(struct hash_algo *algo, void *ctx, void *dest_buf, int size)
{
    if (size < algo->digest_size)
        return -1;
...
#if CONFIG_IS_ENABLED(SHA256) || IS_ENABLED(CONFIG_CMD_SHA1SUM) || CONFIG_IS_ENABLED(CRC32_VERIFY) || \
        IS_ENABLED(CONFIG_CMD_HASH) || CONFIG_IS_ENABLED(SHA384) || CONFIG_IS_ENABLED(SHA512) ||      \
...
int hash_progressive_lookup_algo(const char *algo_name, struct hash_algo **algop)
{
    int i;
...
int hash_block(const char *algo_name, const void *data, unsigned int len, uint8_t *output, int *output_size)
{
    struct hash_algo *algo;
    int ret;
    ...
    if (output_size && *output_size < algo->digest_size) {
        debug("Output buffer size %d too small (need %d bytes)", *output_size, algo->digest_size);
        return -ENOSPC;
    }
    if (output_size)
    ...
    return 0;
                ...
                (defined(CONFIG_CMD_HASH) || defined(CONFIG_CMD_SHA1SUM) || defined(CONFIG_CMD_CRC32)) || \
 ...
 */
static void store_result(struct hash_algo *algo, const uint8_t *sum, const char *dest, int allow_env_vars)
{
    unsigned int i;
    int env_var = 0;
...
int hash_command(const char *algo_name, int flags, struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    ulong addr, len;
...
#if defined(CONFIG_CRC32_VERIFY) || defined(CONFIG_SHA1SUM_VERIFY) || defined(CONFIG_MD5SUM_VERIFY) || \
        defined(CONFIG_HASH_VERIFY)
        if (flags & HASH_FLAG_VERIFY) {
#else
        if (0) {
#endif
            if (parse_verify_sum(algo, *argv, vsum, flags & HASH_FLAG_ENV)) {
            ...
            if (argc) {
                store_result(algo, output, *argv, flags & HASH_FLAG_ENV);
            }
        }
