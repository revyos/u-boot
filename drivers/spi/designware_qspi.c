...
static int request_gpio_cs(struct udevice *bus)
{
	...
	if (ret < 0) {
		dev_err(bus, "Error: %d: Can't get %s gpio!\n", ret, bus->name);
		return ret;
	}
	...
	dev_dbg(bus, "%s: used external gpio for CS management\n", __func__);
#endif
	return 0;
}

static int dw_qspi_of_to_plat(struct udevice *bus)
{
	struct dw_qspi_plat *plat = dev_get_plat(bus);
	...
	plat->frequency = dev_read_u32_default(bus, "spi-max-frequency",
	                                       1000000);
	dev_dbg(bus, "%s: regs=%p max-frequency=%d\n", __func__, plat->regs,
	      plat->frequency);
...
static int dw_qspi_probe(struct udevice *bus)
{
	...
	priv->swap_data = dev_read_u32_default(bus, "spi-swap-data", 0);
	...
	dev_dbg(bus, "ssi_version_id=%c.%c%c%c\n",
	...
	while (max--) {
		...
		if (priv->tx_end - priv->len) {
			if (priv->n_bytes == 1)
				...
				if (priv->swap_data) 
					txw = swab16(*(u16 *)(priv->tx));
				else
					txw = *(u16 *)(priv->tx);
			} else {
				if (priv->swap_data)
					txw = swab32(*(u32 *)(priv->tx));
				else
					txw = *(u32 *)(priv->tx);
			}
		}
		...
		if (priv->rx_end - priv->len) {
			if (priv->n_bytes == 1)
				*(u8 *)(priv->rx) = rxw;
				...
				if (priv->swap_data)
					*(u16 *)(priv->rx) = swab16(rxw);
				else
					*(u16 *)(priv->rx) = rxw;
			} else {
				if (priv->swap_data)
					*(u32 *)(priv->rx) = swab32(rxw);
				else
					*(u32 *)(priv->rx) = rxw;
			}
		       ...
		       const void *dout, void *din, unsigned long flags)
{
	...
	if (bitlen % 8) {
		dev_err(bus, "Non byte aligned SPI transfer.\n");
		return -1;
	}
	...
	/*set quad-mode for test only*/
        cr0 |= 2<<21;
	priv->len = bitlen >> 3;
	dev_dbg(bus, "%s: rx=%p tx=%p len=%d [bytes]\n", __func__, rx, tx, priv->len);
...
static int dw_qspi_exec_op(struct spi_slave *slave, const struct spi_mem_op *op)
{
	...
	int ret;
	struct dw_qspi_plat *plat = NULL;
	plat = dev_get_plat(bus);

	dev_dbg(bus, "%s,reg_base:%p op->data.dir:%d \n",__func__,priv->regs,op->data.dir);
	/*disable spi core */
	spi_enable_chip(priv, 0);
	/*disable slalve*/
	...
	if (op->data.nbytes && op->data.buswidth > 1) {
		...
		dev_dbg(bus, "%s: priv->tx:%p tx_end:%p len:%#x\n", __func__, priv->tx, priv->tx_end, priv->len);
		do {
			dw_writer(priv);
	...
	if (op->data.dir == SPI_MEM_DATA_OUT) {
		...
		dev_dbg(bus, "%s: priv->tx:%p tx_end:%p len:%#x n_bytes:%d\n", __func__, priv->tx, priv->tx_end, priv->len, priv->n_bytes);
		do {
			dw_writer(priv);
                } while (priv->tx_end > priv->tx);
		...
		if (op->data.buswidth == 1) {
			...
			if (ret) {
				dev_err(bus, "wait bus idle time out \n");
				return ret;
			}
			spi_enable_chip(priv, 0);
			...
			dev_dbg(bus, "#2:cr0 %08x cr1 %08x spi_cr0 %08x \n", dw_read(priv, DW_SPI_CTRL0), dw_read(priv, DW_SPI_CTRL1), dw_read(priv, DW_SPI_SPI_CTRLR0));
			spi_enable_chip(priv, 1);
...
static int dw_qspi_set_speed(struct udevice *bus, uint speed)
{
	...
	priv->freq = speed;
	dev_dbg(bus, "%s: regs=%p speed=%d clk_div=%d\n", __func__, priv->regs,
	      priv->freq, clk_div);
...
U_BOOT_DRIVER(dw_qspi) = {
	...
	.ops = &dw_qspi_ops,
	.of_to_plat = dw_qspi_of_to_plat,
	.plat_auto = sizeof(struct dw_qspi_plat),
	.priv_auto = sizeof(struct dw_qspi_priv),
	.probe = dw_qspi_probe,
