...
static void set_slave_cpu_entry(phys_addr_t entry)
{
    writel(entry, (void *)E902_SYSREG_START);
}

static void disable_slave_cpu(void)
{
    writel(0x0, (void *)E902_SYSREG_RESET);
}

static void enable_slave_cpu(void)
{
    writel(0x3, (void *)E902_SYSREG_RESET);
}

static void set_c906_cpu_entry(phys_addr_t entry_h, phys_addr_t entry_l)
{
...
#ifdef CONFIG_LIGHT_AON_CONF
static int get_and_set_aon_config_data(void)
{
	int ret =0;
    struct udevice *dev;
	...
	config_data = (struct mic_regu_platdata *)(dev->plat_);
...
static int do_bootslave(struct cmd_tbl * cmdtp, int flag, int argc, char * const argv[])
{
	boot_audio();
	return 0;
}
