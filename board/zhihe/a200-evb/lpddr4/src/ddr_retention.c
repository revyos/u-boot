...
#define SRAM_E902_BASEADDR 0xFFE0170000
#define DDR_PHY_REG_SAVEADDR (SRAM_E902_BASEADDR)
Ddr_Reg_Config_t *ddr_Regu_Config = (Ddr_Reg_Config_t *)DDR_PHY_REG_SAVEADDR;

#ifndef ARRAY_SIZE
#define ARRAY_SIZE(x) (sizeof(x) / sizeof(x[0]))
#endif

static const uint64_t MiscRegList[] = {
    DFILPCFG0,
    ADDRMAP0,
    ADDRMAP1,
    ADDRMAP2,
...
int dwc_ddrphy_phyinit_regInterface(regInstr myRegInstr,int rank_num) {
    ...
    if (myRegInstr == saveRegs)
    {
        ...
        for (regIndx = 0; regIndx < phy_reg_num; regIndx++)
        {
            data = ddr_phy0_reg_rd(RetRegList_addr[regIndx]);
            phy_addr_t[regIndx].Value0 = data;
            phy_addr_t[regIndx].Address = RetRegList_addr[regIndx];
        }
#ifndef CONFIG_DDR_H32_MODE
        for (regIndx = 0; regIndx < phy_reg_num; regIndx++)
        {
            data = ddr_phy1_reg_rd(RetRegList_addr[regIndx]);
            phy_addr_t[regIndx].Value1 = data;
        }
#endif
    }
