...
void lp_ccu_reg_init_with_gating(u64 base_addr, u32 reg_dly_time, u32 reg_dly_time_step)
{
	...
	//1.open interrupt
	chip_wr(base_addr + LP_REG_CCU_INT_EN_ADDR_OFFSET, 1);
	//2.open auto clock gating
	data = chip_rd(base_addr + LP_REG_CCU_CTRL_ADDR_OFFSET);
	data = data | (1 << LP_REG_CCU_AUTOGATE_FIELD_OFFSET);
	chip_wr(base_addr + LP_REG_CCU_CTRL_ADDR_OFFSET, data);
	chip_wr(base_addr + LP_REG_CCU_TIME_ADDR_OFFSET,
...
void venc_cclk_config(unsigned int freq)
{
	...
	switch (freq) {
	case 528:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x38);
		tmp &= ~0xF3;
		chip_wr(AP_TOP_CRG_BADDR + 0x38, tmp | 0x51);
		break;
	case 600:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x38);
		tmp &= ~0xF3;
		chip_wr(AP_TOP_CRG_BADDR + 0x38, tmp | 0x50);
		break;
	default:
...
void vdec_cclk_config(unsigned int freq)
{
	...
	case 660:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x38);
		tmp &= ~0xF300;
		chip_wr(AP_TOP_CRG_BADDR + 0x38, tmp | 0x4100);
		break;
	case 786:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x38);
		tmp &= ~0xF300;
		chip_wr(AP_TOP_CRG_BADDR + 0x38, tmp | 0x3000);
		break;
	default:
		printf("wrong freq for %s!\n", __func__);
		break;
...
void vp_aclk_config(unsigned int freq)
{
	...
	case 786:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x38);
		tmp &= ~0x1F000000;
		chip_wr(AP_TOP_CRG_BADDR + 0x38, tmp | 0x13000000);
		break;
	case 880:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x38);
		tmp &= ~0x1F000000;
		chip_wr(AP_TOP_CRG_BADDR + 0x38, tmp | 0x3000000);
		break;
	default:
		printf("wrong freq for %s!\n", __func__);
		break;
...
void g2d_cclk_config(unsigned int freq)
{
	...
	case 660:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x38);
		tmp &= ~0xF10000;
		chip_wr(AP_TOP_CRG_BADDR + 0x38, tmp | 0x420000);
		break;
	case 786:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x38);
		tmp &= ~0xF10000;
		chip_wr(AP_TOP_CRG_BADDR + 0x38, tmp | 0x300000);
		break;
	default:
		printf("wrong freq for %s!\n", __func__);
		break;
...
void gpu_cclk_config(unsigned int freq)
{
	...
	switch (freq) {
	case 660:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x18);
		tmp &= ~0xF3;
		chip_wr(AP_TOP_CRG_BADDR + 0x18, tmp | 0x41);
		break;
	case 792:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x18);
		tmp &= ~0xF3;
		chip_wr(AP_TOP_CRG_BADDR + 0x18, tmp | 0x33);
		break;
	default:
		printf("wrong freq for %s!\n", __func__);
		break;
...
void npu_cclk_config(unsigned int freq)
{
	...
	switch (freq) {
	case 330:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF3;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x81);
		break;
	case 660:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF3;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x41);
		break;
	case 786:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF3;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x32);
		break;
	case 880:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF3;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x31);
		break;
	case 1000:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF3;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x30);
		break;
	default:
		printf("wrong freq for %s!\n", __func__);
		break;
...
void npu_aclk_config(unsigned int freq)
{
	...
	case 330:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF300;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x8100);
		break;
	case 660:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF300;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x4100);
		break;
	case 786:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF300;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x3200);
		break;
	case 880:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF300;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x3100);
		break;
	case 1000:
		tmp = chip_rd(AP_TOP_CRG_BADDR + 0x3c);
		tmp &= ~0xF300;
		chip_wr(AP_TOP_CRG_BADDR + 0x3c, tmp | 0x3000);
		break;
	default:
		printf("wrong freq for %s!\n", __func__);
		break;
...
{
	chip_wr(AP_TOP_CRG_BADDR + 0x0, 0x2300202); // TOP_ONLY_CLK_CFG
	chip_wr(AP_TOP_CRG_BADDR + 0x4, chip_rd(AP_TOP_CRG_BADDR + 0x4) | (0x3 << 20)); // CPU_CLK_CFG_0
	chip_wr(AP_TOP_CRG_BADDR + 0x8, chip_rd(AP_TOP_CRG_BADDR + 0x8) | (0x1 << 16)); // CPU_CLK_CFG_1 clkgen_sys_bus_clk=1.32g

	chip_wr(AP_TOP_CRG_BADDR + 0x18, (chip_rd(AP_TOP_CRG_BADDR + 0x18) & 0xfffff0ff) | (0x4 << 8)); // GPU_CLK_CFG
	chip_wr(AP_TOP_CRG_BADDR + 0x14, chip_rd(AP_TOP_CRG_BADDR + 0x14) | (0x3 << 4)); // D2D_CLK_CFG

	chip_wr(AP_TOP_CRG_BADDR + 0x20, (chip_rd(AP_TOP_CRG_BADDR + 0x20) & 0xfff8ffff) | (0x2 << 16)); // PERI_CLK_CFG1
	chip_wr(AP_TOP_CRG_BADDR + 0x2c, chip_rd(AP_TOP_CRG_BADDR + 0x2c) | (0x1 << 20)); // PERI_CLK_CFG4
	chip_wr(AP_TOP_CRG_BADDR + 0x28, (chip_rd(AP_TOP_CRG_BADDR + 0x28) & 0xf7ffff00) | (0x63)); // PERI_CLK_CFG3

	chip_wr(AP_TOP_CRG_BADDR + 0x34, chip_rd(AP_TOP_CRG_BADDR + 0x34) | (0x2 << 8)); // USB_CLK_CFG

	udelay(1);

	chip_wr(AP_PLL_WRAP_BADDR + 0x140, 0x1106e01); // VIDEO_PLL_CFG0
	chip_wr(AP_PLL_WRAP_BADDR + 0x144, 0x1000000); // VIDEO_PLL_CFG1
	chip_wr(AP_PLL_WRAP_BADDR + 0x40, 0x1107d01); // GMAC_PLL_CFG0
	chip_wr(AP_PLL_WRAP_BADDR + 0x44, 0x1000000); // GMAC_PLL_CFG1
	chip_wr(AP_PLL_WRAP_BADDR + 0x0, 0x1106201); // AUDIO0_PLL_CFG0
	chip_wr(AP_PLL_WRAP_BADDR + 0x4, 0x4aaaab); //AUDIO0_PLL_CFG1
	chip_wr(AP_PLL_WRAP_BADDR + 0x20, 0x1106901); //AUDIO1_PLL_CFG0
	chip_wr(AP_PLL_WRAP_BADDR + 0x24, 0x555555); //AUDIO1_PLL_CFG1
...
void bpc_config(char *str, u32 base_addr, u32 bpc_ctrl)
{
	if ((bpc_ctrl & (1 << 0)) != 0) {
		...
		chip_wr(base_addr + 0x000, 0x1); // 0x1 bypass
		chip_wr(base_addr + 0x13c, 0x18); // bpc 9000 ocgen &rset
	} else {
		debug("Enter %s bpc_config hw model...\n", str);
		chip_wr(base_addr + 0x000, 0x0);
	}
	chip_wr(base_addr + 0x004, 0x10101); // pwr venc bpc 3000| fence
}
...
void pcu_intr(char *str, u32 base_addr)
{
	u32 data;
	udelay(1);
	data = chip_rd(base_addr + 0x2c); // read pcu intr
	while (data == 0) {
		udelay(1);
		data = chip_rd(base_addr + 0x2c); // read pcu intr
	}
	...
	chip_wr(base_addr + 0x28, data); // clr cpu intr
}

void pcu_config(char *str, u32 base_addr, u32 pcu_ctrl, u32 state)
{
	debug("Enter %s pcu_config intr enable...\n", str);
	chip_wr(base_addr + 0x24, 0x3f); // interrupt enable
	if ((pcu_ctrl & (1 << 0)) != 0) {
		debug("Enter %s pcu_config: pcu reg trigger...\n", str);
		chip_wr(base_addr + 0x0c, (state & 0x1f)); // lpstate = power on
		chip_wr(base_addr + 0x08, 0x1); // lqreq
...
void vpss_r2p_intr(u32 r2p_ctrl)
{
	...
	if ((r2p_ctrl & (1 << 0)) == 0) {
		udelay(1);
		data = chip_rd(AP_VP_PTRL_R2P_BADDR + 0x1c); // rd r2p intr
		while (data == 0) {
			udelay(1);
			data = chip_rd(AP_VP_PTRL_R2P_BADDR + 0x1c); // rd r2p intr
		}
		...
		chip_wr(AP_VP_PTRL_R2P_BADDR + 0x14, data); // clr pwr pcu intr
	} else {
...
void vpss_r2p_low_power_config(u32 r2p_ctrl, u32 pctrl_en)
{
	...
	chip_wr(AP_VP_PTRL_R2P_BADDR + 0x10, 0x7); // r2p intr en
	if ((r2p_ctrl & (1 << 0)) == 0) {
		chip_wr(AP_VP_PTRL_R2P_BADDR + 0x08,
		   (0x00001f << 4) | ((pctrl_en & 0x7) << 1) | (order & 0x1)); // r2p
		chip_wr(AP_VP_PTRL_R2P_BADDR + 0x04, 0x01);
...
void viss_low_power_config(u32 pcu, u32 bpc, power_mode enable)
{
	if (ON == enable) {
		...
		chip_wr(AP_VI_PCTRL_R2P_BADDR + 0x10, 0x7); // r2p intr en
	} else {
		bpc_config("vi_isp_bpc", AP_VI_PCTRL_ISP_PTRL_BPC_BADDR, bpc);
		pcu_config("vi_isp_pcu", AP_VI_PCTRL_ISP_PTRL_PCU_BADDR, pcu, enable);
		bpc_config("vi_wrap_bpc", AP_VI_PCTRL_VI_WRAP_PTRL_BPC_BADDR, bpc);
...
{
	debug("%s\n", __func__);
	chip_wr(AP_VP_PTRL_PCA_BADDR + 0x20, 0x0); // pca off
	vpss_low_power_config(PCU_REG_TRIGGER, BPC_HW_MODEL, ON);
	chip_wr(AP_VP_SYSREG_BADDR + 0x200, 0xffffffff); // VP_CLK_EN
	chip_wr(AP_VP_SYSREG_BADDR + 0x400, 0xffffffff); // VP_RSTN
	udelay(1);
	//pmic ctrl
	chip_wr(AP_AON_PMIC_CTRL_BADDR + 0x50, chip_rd(AP_AON_PMIC_CTRL_BADDR + 0x50) | 24);
	chip_wr(AP_AON_PMIC_CTRL_BADDR + 0x5c, AON_AON_I2C0_BADDR + 0x04);
	chip_wr(AP_AON_PMIC_CTRL_BADDR + 0x58, AON_AON_I2C0_BADDR + 0x10);
...
void vpss_cpr_deinit(void)
{
	...
	chip_wr(AP_VP_SYSREG_BADDR + 0x200, 0x0); // VP_CLK_EN
	chip_wr(AP_VP_SYSREG_BADDR + 0x400, 0x0); // VP_RSTN
}
...
void viss_cpr_init(void)
{
	...
	chip_wr(AP_VI_SYSREG_BADDR + 0x200, 0xFFFFFFFF); // VI_CLK_EN
	chip_wr(AP_VI_SYSREG_BADDR + 0x204, 0xFFFFFFFF); // VI_CLK_EN2
	chip_wr(AP_VI_SYSREG_BADDR + 0x50, chip_rd(AP_VI_SYSREG_BADDR + 0x50) | 0x7ff); // isp internal clk
	chip_wr(AP_VI_SYSREG_BADDR + 0x400, 0xFFFFFFFF); // VI_RSTN
	udelay(1);
...
void viss_cpr_deinit(void)
{
	...
	chip_wr(AP_VI_SYSREG_BADDR + 0x200, 0); // VI_CLK_EN
	chip_wr(AP_VI_SYSREG_BADDR + 0x204, 0); // VI_CLK_EN2
	chip_wr(AP_VI_SYSREG_BADDR + 0x400, 0); // VI_RSTN
}
...
void vo_cpr_init(void)
{
	...
	chip_wr(AP_VO_SYSREG_BADDR + 0x200, 0xFFFFFFFF); // VO_CLK_EN
	chip_wr(AP_VO_SYSREG_BADDR + 0x400, 0xFFFFFFFF); // VO_RSTN
	chip_wr(AP_VO_SYSREG_BADDR + 0x48, 0x4); //dpu port0 link dsi, dpu port1 link hdmi
	udelay(1);
...
void vo_cpr_deinit(void)
{
	...
	chip_wr(AP_VO_SYSREG_BADDR + 0x200, 0x0); // VO_CLK_EN
	chip_wr(AP_VO_SYSREG_BADDR + 0x400, 0x0); // VO_RSTN
}

void npu_low_power_config(u32 pcu, u32 bpc, power_mode enable)
{
	if (chip_rd(NPU_INDICATOR) == 0xdead) {
		...
		chip_wr(0x07112070, 0xffff);
		chip_wr(0x07112074, 0);
		chip_wr(0x0711207c, 1);
...
{
	debug("%s\n", __func__);
	chip_wr(AON_NPU_PCTRL_PCA_BADDR + 0x20, 0x0); // pca off
	npu_low_power_config(PCU_REG_TRIGGER, BPC_HW_MODEL, ON);
	//cfg rst/clk register
	chip_wr(AP_NPU_CRG_BADDR + 0x200, 0xffffffff);
	chip_wr(AP_NPU_CRG_BADDR + 0x210, 0xffffffff);
	chip_wr(AP_NPU_CRG_BADDR + 0x074, 0x0000001);
	chip_wr(AP_NPU_CRG_BADDR + 0x07c, 0x0000001);
...
void pcie_low_power_config(u32 pcu, u32 bpc, power_mode enable)
{
	...
	if (enable == ON) {
		chip_wr(AP_PERI1_PADCTRL_BADDR + 0x40c, 0x55055555);
		chip_wr(AON_PCIE_CPR_BADDR + 0x0, 0x1111);
		chip_wr(AON_PCIE_CPR_BADDR + 0x4, 0x111);
		chip_wr(AON_PCIE_CPR_BADDR + 0x8, 0x1);
		chip_wr(AON_PCIE_CPR_BADDR + 0x10, 0x110111);
		chip_wr(AON_PCIE_CPR_BADDR + 0x14, 0x110001);
		chip_wr(AON_PCIE_CPR_BADDR + 0x20, 0x111111);
		chip_wr(AON_PCIE_CPR_BADDR + 0x24, 0x111111);
		chip_wr(AON_PCIE_CPR_BADDR + 0x100, 0x1111);
		chip_wr(AON_PCIE_CPR_BADDR + 0x104, 0x11);
		chip_wr(AON_PCIE_CPR_BADDR + 0x108, 0x10);
		chip_wr(AON_PCIE_CPR_BADDR + 0x110, 0x111111);
		chip_wr(AON_PCIE_CPR_BADDR + 0x114, 0x1111);
		chip_wr(AON_PCIE_CPR_BADDR + 0x120, 0x110011);
		chip_wr(AON_PCIE_CPR_BADDR + 0x124, 0x110011);
	}
}
...
void pcie_cpr_init(void)
{
	...
	chip_wr(AP_AON_PADCTRL_BADDR + 0x414, (chip_rd(AP_AON_PADCTRL_BADDR + 0x414) & 0xffffff0f) | (0x1 << 4));
	chip_wr(AP_PERI1_PADCTRL_BADDR + 0x404,
	   (chip_rd(AP_PERI1_PADCTRL_BADDR + 0x404) & 0xfff0000f) | (0x5555 << 4));
	chip_wr(AP_PERI1_PADCTRL_BADDR + 0x40c,
	   (chip_rd(AP_PERI1_PADCTRL_BADDR + 0x40c) & 0xffff) | (0x5555 << 16));

	chip_wr(AP_PCIE_PCTRL_E16PHY0_PCA_BADDR + 0x20, 0x0); // pca off
	chip_wr(AP_PCIE_PCTRL_E16PHY1_PCA_BADDR + 0x20, 0x0); // pca off
...
void usb_cpr_init(unsigned int ss_cfg)
{
	...
	udelay(1);
	chip_wr(AP_USB_CPR_BADDR + 0x4, 0xffffffff);
	chip_wr(AP_USB_CPR_BADDR + 0x8, 0xffffffff);
	chip_wr(AP_USB_CPR_BADDR + 0xc, 0xffffffff);
	chip_wr(AP_USB_CPR_BADDR + 0x104, 0xffffffff);
	chip_wr(AP_USB_CPR_BADDR + 0x108, 0xffffffff);
	chip_wr(AP_USB_CPR_BADDR + 0x10c, 0xffffffff);
	if (ss_cfg & (1 << PERI_CPR)) {
		chip_wr(AP_USB_CPR_BADDR + 0x20, 0x1); // PERI2 CLK_EN on
		chip_wr(AP_USB_CPR_BADDR + 0x120, 0x1); // PERI2 SWRST on
	}
...
void aon_low_power_config(u32 pcu, u32 bpc)
{
	...
	chip_wr(AP_AON_PCTRL_PCU_BADDR + 0x6c, 0); // LP_REG_PCU_DEVICE_ENABLE_HIGH2LOW_OFFSET on -> off
	chip_wr(AP_AON_PCTRL_PCU_BADDR + 0x70, 0); // LP_REG_PCU_DEVICE_ENABLE_LOW2HIGH_OFFSET
...
{
	uint32_t tmp;
	tmp = chip_rd(AP_TEE_SYSREG_BADDR + 0x1c);
	tmp &= (~0x1);
	chip_wr(AP_TEE_SYSREG_BADDR + 0x1c, tmp);
}
...
int lp_ddr_ss_ppu_init_by_addr(u64 base_addr, u32 reg_pst_switch_dly_time_2)
{
	...
	//1. wr pm enable
	data = chip_rd(base_addr + LP_REG_PPU_CTRL_ADDR_OFFSET);
	data = (data | 0x1000) & 0xffffb0ff; // DSLP
	chip_wr(base_addr + LP_REG_PPU_CTRL_ADDR_OFFSET, data);
	//2. wr time
	// chip_wr(base_addr+LP_REG_PPU_DLY_TIME_CTRL_ADDR_OFFSET,0x0); // reg_pst_switch_dly_time_step_2=0x1
	// chip_wr(base_addr+LP_REG_PPU_DLY_TIME_0_ADDR_OFFSET,0x55);
	// chip_wr(base_addr+LP_REG_PPU_DLY_TIME_1_ADDR_OFFSET,0x55);
	chip_wr(base_addr + LP_REG_PPU_DLY_TIME_2_ADDR_OFFSET, reg_pst_switch_dly_time_2);
	//3. wr dyn mode
	data = chip_rd(base_addr + LP_REG_PPU_CTRL_ADDR_OFFSET);
	data = data | 0x1;
	chip_wr(base_addr + LP_REG_PPU_CTRL_ADDR_OFFSET, data);
...
{
	u32 data;
	chip_wr(0x06e00620, 0x0);
	data = chip_rd(AP_AON_PMIC_CTRL_BADDR + 0x50);
	data = data | 24;
	chip_wr(AP_AON_PMIC_CTRL_BADDR + 0x50, data);
	chip_wr(AP_AON_PMIC_CTRL_BADDR + 0x5c, 0x3084a000 + 0x4);
	chip_wr(AP_AON_PMIC_CTRL_BADDR + 0x58, 0x3084a000 + 0x10);
...
void config_iopmp(u32 chip_id, u64 iopmp_base_addr, u32 mode, u32 devid, u32 permission, u32 index)
{
	...
	if (mode == IOPMP_MODE_TOR) {
		chip_wr(iopmp_base_addr + index * 4, permission); //TOR for entry index*4~index*4+1

		chip_wr(iopmp_base_addr + 0x800 + index * 0x20, 0x0);
		chip_wr(iopmp_base_addr + 0x804 + index * 0x20,
		   (global << 31) | (devid << 14)); //PPN=0x0 for entry index*4

		chip_wr(iopmp_base_addr + 0x808 + index * 0x20, 0x0);
		chip_wr(iopmp_base_addr + 0x80C + index * 0x20,
		   0x20 | (global << 31) | (devid << 14)); //PPN=0x80_0000_0000 for entry index*4+1
	} else {
		chip_wr(iopmp_base_addr + index * 4, permission); //NAPOT for entry index*4

		chip_wr(iopmp_base_addr + 0x800 + index * 0x20, 0xFFFFFFFF);
		chip_wr(iopmp_base_addr + 0x804 + index * 0x20,
		   (global << 31) | (devid << 14) |
